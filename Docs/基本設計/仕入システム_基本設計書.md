...
| 仕入システム ロードマップ | `Docs/ロードマップ/仕入システム_ロードマップ.md` | 1.6 |
...
| フロントエンド | Laravel標準構成を前提とし、Blade中心またはVue.js/React採用をPhase 1の最初に確定する |
...
| 帳票 | PDF/Excel/CSV出力を対象とし、採用ライブラリは帳票実装前に確定する |
...
| 認証・権限 | `role_permissions` | ロール権限関連 |
...
| バッチ | `job_execution_logs` | バッチ実行履歴 |
| バッチ | `failed_jobs` | Laravel標準のキュー失敗管理 |
| 監査 | `audit_logs` | 操作ログ |
...
本書ではシステム全体の基本構造を定義する。以下は詳細設計で定義済みであり、実装時は各詳細設計書を参照する。
...
| 精算支払詳細設計 | 精算、支払予定、支払確定、全銀協形式CSVの処理仕様 |
...
| 2026年5月14日 | 1.2 | 詳細設計書作成に伴い、参照ロードマップの版数を更新 |
| 2026年5月14日 | 1.3 | 詳細設計書再帰レビューに伴い、精算支払、認証権限、バッチの主要テーブルと詳細設計対象を補足 |
| 2026年5月15日 | 1.4 | 実装前ロードマップ確認に伴い、参照ロードマップの版数を1.5へ更新 |
| 2026年5月15日 | 1.5 | 実装前全体レビューに伴い、フロントエンド確定タイミング、帳票ライブラリ選定タイミング、詳細設計参照方針を補正 |
