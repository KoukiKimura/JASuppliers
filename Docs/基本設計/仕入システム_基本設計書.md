...
| 仕入システム ロードマップ | `Docs/ロードマップ/仕入システム_ロードマップ.md` | 1.1 |
...
| 精算・支払 | 月次締め、支払予定、支払明細、振込依頼CSV | C-006、C-008、CC-010、B-009 |
| 帳票・会計連携 | PDF/Excel帳票、仕訳CSV、振込依頼CSV | 3.4、3.5 |
...
| MST-004 | 手数料・掛率・支払条件管理 | 委託手数料率、消化掛率、支払条件の管理 | C-010、CC-007、B-009 |
...
| PAY-001 | 支払管理 | 支払予定、支払確定、振込CSV出力 | C-008、CC-010、B-009 |
...
- 業務データには原則として論理削除用の `deleted_at` を設ける。
- 外部キー制約を定義する。
...
| 外部出力 | `bank_transfer_exports` | 振込依頼CSV出力履歴 |
...
| 重複処理 | 一意キーが既存の場合は二重計上せず、更新またはスキップ方針を詳細設計で確定する |
...
| PAYMENT_EXPORT | 支払・振込CSV出力 | 手動実行 | 支払確定データから振込依頼CSVを出力する |
...
| 経理担当者 | 精算、請求照合、支払、帳票、会計・振込CSV出力 |
...
| バックアップ | PostgreSQLのバックアップ手順は実装フェーズで手順化する |
...
| 帳票・CSV詳細設計 | PDF、Excel、仕訳CSV、振込CSVの項目定義 |
