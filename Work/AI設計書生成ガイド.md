...
- テーブル設計はMySQL/PostgreSQL互換のDDLを含めること
...
3. インデックス定義
4. CREATE TABLE文（MySQL 8.0対応DDL）
...
制約: MySQL 8.0、正規化は第3正規形以上、トレーサビリティの確保が必須
...
### 1. テーブル定義書
以下のテーブルをそれぞれ定義（カラム定義表 + MySQL 8.0 DDL）:
- t_supplier（出荷者マスタ）
- t_product（商品マスタ）
...
- [ ] DDLが実際に実行可能な構文か確認する（MySQL Workbench等で検証）
...
## 使用技術スタック（想定）
- バックエンド: Java (Spring Boot) または PHP (Laravel)
